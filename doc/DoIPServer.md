...
The relevant example source files are located in the `examples/discover/` directory:

- `examples/discover/DoIPServer.cpp` — program entry point and socket setup.
- `examples/discover/DoIPServerModel.h` — example `DoIPServerModel` with
...
- `examples/discover/DoIPServerModel.h` — shows a concrete `DoIPServerModel`
...
- `examples/discover/DoIPServer.cpp` — creates and configures a `DoIPServer`,
...
On success the example binaries are available under `build/examples/discover/`.

## Running the example server

The example executable is `DoIPServer`. Run it from the build
directory, optionally enabling loopback mode (binds announcements to
127.0.0.1 instead of broadcast):

```bash
./examples/discover/DoIPServer [--loopback]
```
...
- `DoIPServer` exits immediately with critical log about sockets:
...
If you generate documentation with Doxygen, the file `doc/DoIPServer.md`
